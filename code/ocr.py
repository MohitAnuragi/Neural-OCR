...
class OCRNeuralNetwork:
    LEARNING_RATE = 0.1
    WIDTH_IN_PIXELS = 20
    NN_FILE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'nn.json')
    ...
    def train(self, training_data_array):
        for data in training_data_array:
            # Support both namedtuple attribute access (initial training) and
            # dict subscript access (server-driven incremental training).
            ...
            self.theta1 += self.LEARNING_RATE * np.dot(np.asmatrix(hidden_errors), np.asmatrix(y0))
