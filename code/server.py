...
class JSONHandler(BaseHTTPRequestHandler):
    def do_POST(s):
        ...
        if payload.get('train'):
            invalid = [item['label'] for item in payload['trainArray']
                       if not isinstance(item['label'], int) or not (0 <= item['label'] <= 9)]
            if invalid:
                response_code = 400
                response = {"type": "error", "message": "Invalid label(s): {}. Labels must be integers 0-9.".format(invalid)}
            else:
